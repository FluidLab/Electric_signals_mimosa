...
class HHModel(Gate):
    ...
    def __init__(self, startingVoltage=-0.147):
...
class Simulation:
    ...
    def Run(self, stimulusWaveform, Cl_conductance, stepSizeMs):
        if (stepSizeMs > 0.05):
        ...
        for i in range(len(stimulusWaveform)):
            self.model.iterate(stimulusWaveform[i], Cl_conductance[i], stepSizeMs)
